...
- (instancetype)initWithChatter:(NSString *)chatter conversationType:(EMConversationType)type
{
    ...
    if (self) {
        ...
        self.conversation = [[EaseMob sharedInstance].chatManager conversationForChatter:chatter conversationType:type];
...
- (void)viewDidLoad
{
    ...
    if (self.isShare) {
        self.chatToolBar.inputTextView.text = self.shareContent;
...
- (DXMessageToolBar *)chatToolBar
{
    if (_chatToolBar == nil) {
        ...
        _chatToolBar.moreView.backgroundColor =  [UIColor colorWithRed:238/255.0 green:238/255.0 blue:238/255.0 alpha:1.0f];
...
- (void)didStartRecordingVoiceAction:(UIView *)recordView
{
    if ([self canRecord]) {
        ...
        NSInteger x = arc4random() % 100000;
        NSTimeInterval time = [[NSDate date] timeIntervalSince1970];
        NSString *fileName = [NSString stringWithFormat:@"%ld%ld",(long)time,(long)x];
