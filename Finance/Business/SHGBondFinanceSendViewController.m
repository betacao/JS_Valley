...
- (void)initView
{
    //NSString *source = @"";//资金来源
    NSString *bondType = @"";//业务类型
    NSString *industry = @"";//所属行业
    if (self.sendType == 1) {
        ...
        if (bondType.length > 0) {
             bondType = [key objectAtIndex:[value indexOfObject:bondType]];
        } else{
            bondType = @"";
        }
...
//    NSString * str = @"realtyConstruction;financial;energy;internet";
