...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        _doneButton = [[UIBarButtonItem alloc] initWithTitle:@"" style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)];
        // Set appearance
...
- (void)updateNavigation {
    ...
    UILabel *colorLabel=[[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.navigationItem.titleView.bounds.size.width, 30)];
    colorLabel.textColor=[UIColor whiteColor];
	// Title
    NSUInteger numberOfPhotos = [self numberOfPhotos];
    if (_gridController) {
        if (_gridController.selectionMode) {
            colorLabel.text = NSLocalizedString(@"Select Photos", nil);
        } else {
            NSString *photosText;
            if (numberOfPhotos == 1) {
            ...
            colorLabel.text = [NSString stringWithFormat:@"%lu %@", (unsigned long)numberOfPhotos, photosText];
        }
    } else if (numberOfPhotos > 1) {
        if ([_delegate respondsToSelector:@selector(photoBrowser:titleForPhotoAtIndex:)]) {
            colorLabel.text = [_delegate photoBrowser:self titleForPhotoAtIndex:_currentPageIndex];
        } else {
            colorLabel.text = [NSString stringWithFormat:@"%lu %@ %lu", (unsigned long)(_currentPageIndex+1), NSLocalizedString(@"of", @"Used in the context: 'Showing 1 of 3 items'"), (unsigned long)numberOfPhotos];
        }
	} else {
		colorLabel.text = @"";
	}
    
    self.navigationItem.titleView=colorLabel;
                ...
                if (SYSTEM_VERSION_LESS_THAN(@"6")) {
                    ...
                    [self.activityViewController setCompletionHandler:^(NSString *activityType, BOOL completed) {
                        if (completed) {
                            ...
                            [weakSelf showProgressHUDWithMessage:message];
                        }
...
- (void)showProgressHud:(NSString *)message {
    ...
    [self.progressHUD hide:YES afterDelay:1.2];
    self.navigationController.navigationBar.userInteractionEnabled = NO;
}
